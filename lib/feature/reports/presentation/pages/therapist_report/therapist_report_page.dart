...
import 'package:ali_therapy_admin/feature/reports/presentation/widgets/other/therapist_report_totals.dart';
...
class TherapistReportPage extends StatelessWidget {
  ...
  TherapistReportSummaryEntity _summaryOf(TherapistReportState state) {
    if (state is TherapistReportLoaded) return state.summary;
    if (state is TherapistReportError) return state.summary;
    return const TherapistReportSummaryEntity.empty();
  }

  bool _showStatsOf(TherapistReportState state) {
    if (state is TherapistReportLoaded) return state.showStats;
    if (state is TherapistReportError) return state.showStats;
    return false;
  }
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<TherapistReportBloc, TherapistReportState>(
        ...
        builder: (context, state) {
          final isFirstLoad =
              state is TherapistReportLoading ||
