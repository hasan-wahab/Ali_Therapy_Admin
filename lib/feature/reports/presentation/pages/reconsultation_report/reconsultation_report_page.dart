...
import 'package:ali_therapy_admin/feature/reports/presentation/widgets/other/reconsultation_report_totals.dart';
...
class ReconsultationReportPage extends StatelessWidget {
  ...
  ReconsultationReportSummaryEntity _summaryOf(
    ReconsultationReportState state,
  ) {
    if (state is ReconsultationReportLoaded) return state.summary;
    if (state is ReconsultationReportError) return state.summary;
    return const ReconsultationReportSummaryEntity.empty();
  }

  bool _showStatsOf(ReconsultationReportState state) {
    if (state is ReconsultationReportLoaded) return state.showStats;
    if (state is ReconsultationReportError) return state.showStats;
    return false;
  }
          ...
          if (!isFirstLoad && _showStatsFromApi)
            SliverPadding(
              ...
              sliver: SliverToBoxAdapter(
                child: ReconsultationReportTotals(
