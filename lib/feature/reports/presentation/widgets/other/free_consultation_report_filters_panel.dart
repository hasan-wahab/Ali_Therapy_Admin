...
// ============================================================
// FREE CONSULTATION REPORT FILTERS PANEL
// ------------------------------------------------------------
...
class FreeConsultationReportFiltersPanel extends StatefulWidget {
  const FreeConsultationReportFiltersPanel({
    super.key,
  ...
  final FreeConsultationReportQuery currentQuery;
  final VoidCallback? onApplied;

  @override
  State<FreeConsultationReportFiltersPanel> createState() =>
      _FreeConsultationReportFiltersPanelState();
}

class _FreeConsultationReportFiltersPanelState
    extends State<FreeConsultationReportFiltersPanel> {
  ...
  @override
  void didUpdateWidget(covariant FreeConsultationReportFiltersPanel oldWidget) {
  ...
  void _syncFromQuery(FreeConsultationReportQuery q) {
    ...
    _perPage = FreeConsultationReportPerPage.labelFor(q.perPage);
  }
  ...
  FreeConsultationReportQuery _draftQuery() {
    return FreeConsultationReportQuery(
      ...
      perPage: FreeConsultationReportPerPage.valueForLabel(_perPage),
      page: 1,
    );
  }

  FreeConsultationReportQuery get _normalizedApplied =>
      FreeConsultationReportQuery(
        search: widget.currentQuery.search,
  ...
  void _applyFilters() {
    ...
    context.read<FreeConsultationReportBloc>().add(
          FreeConsultationReportFiltersApplied(
  ...
  Widget _fromDateField() {
    return ReportDateField(
      key: ValueKey('fc_from_$_resetToken'),
      label: 'From Date',
  ...
  Widget _toDateField() {
    return ReportDateField(
      key: ValueKey('fc_to_$_resetToken'),
      label: 'To Date',
  ...
  Widget _consultantField() {
    return AppDropdownField(
      compact: true,
      key: ValueKey('fc_consultant_$_resetToken'),
      label: 'Consultant',
  ...
  Widget _clinicField() {
    return AppDropdownField(
      compact: true,
      key: ValueKey('fc_clinic_$_resetToken'),
      label: 'Clinic',
  ...
  Widget _perPageField() {
    return AppDropdownField(
      compact: true,
      key: ValueKey('fc_per_page_$_resetToken'),
      label: 'Per Page',
      hintText: FreeConsultationReportPerPage.defaultSize.toString(),
      items: FreeConsultationReportPerPage.dropdownLabels,
      value: FreeConsultationReportPerPage.dropdownLabels.contains(_perPage)
          ? _perPage
          : FreeConsultationReportPerPage.labelFor(
              FreeConsultationReportPerPage.defaultSize,
            ),
      onChanged: (v) {
        if (v == null) return;
        setState(() => _perPage = v);
