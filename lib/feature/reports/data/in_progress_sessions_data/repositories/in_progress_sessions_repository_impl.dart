...
// ============================================================
// IN-PROGRESS SESSIONS REPOSITORY IMPL (Data)
// ============================================================

class InProgressSessionsRepositoryImpl implements InProgressSessionsRepository {
  ...
  ResultFuture<InProgressSessionsPageEntity> getInProgressSessionsPage({
    ...
    if (!await networkInfo.ensureConnected()) {
      ...
      AppErrorLogger.logFailure(
        failure,
        where: 'InProgressSessionsRepository.getInProgressSessionsPage',
      );
      return Result.failure(failure);
    }

    try {
      final model = await remoteDataSource.getInProgressSessionsPage(
        query: query,
      );
