...
// ============================================================
// PACKAGE ATTENDANCE REPOSITORY IMPL (Data)
// ============================================================
  ...
  ResultFuture<PackageAttendancePageEntity> getPackageAttendancePage({
    ...
    if (!await networkInfo.ensureConnected()) {
      ...
      AppErrorLogger.logFailure(
        failure,
        where: 'PackageAttendanceRepository.getPackageAttendancePage',
      );
    ...
    try {
      final model =
          await remoteDataSource.getPackageAttendancePage(query: query);
